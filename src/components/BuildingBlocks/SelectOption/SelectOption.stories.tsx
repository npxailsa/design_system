...
import { SelectOptionDocs } from './SelectOptionDocs';
import type { SelectOptionSize, SelectOptionPosition, SelectOptionSelectionType } from './SelectOption';

const meta: Meta<typeof SelectOption> = {
  title: 'Foundation/BuildingBlocks/SelectOption',
  component: SelectOption,
  parameters: {
    docs: {
      page: () => <SelectOptionDocs />,
    },
  },
  argTypes: {
    size: {
      ...
      return (
        <div style={{ maxWidth: '280px', padding: '24px' }}>
          {options.map((opt, i) => {
            ...
            const pos: SelectOptionPosition =
...
export const StatusDisabled: Story = {
  ...
  render: () => (
    ...
    >
      {(['none', 'checkbox', 'radio'] as SelectOptionSelectionType[]).map((type) => (
        <div key={type}>
          <div
            style={{
              fontFamily: 'var(--brand-font-primary)',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', padding: '24px' }}>
      ...
      <div>
        ...
        <div style={{ maxWidth: '280px' }}>
          {(['top', 'mid', 'bottom'] as SelectOptionPosition[]).map((pos, i) => (
            <SelectOption key={pos} label={`Position: "${pos}"`} position={pos} selectionType="checkbox" selected={i === 1} />
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxWidth: '280px' }}>
          {(['none', 'checkbox', 'radio'] as SelectOptionSelectionType[]).map((type) => (
            <SelectOption key={type} label={`selectionType="${type}"`} selectionType={type} position="solo" />
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxWidth: '280px' }}>
          {(['small', 'default', 'large'] as SelectOptionSize[]).map((size) => (
            <SelectOption key={size} label={`size="${size}"`} size={size} position="solo" selectionType="radio" />
          ))}
        </div>
      </div>
    </div>
