...
export type WarningButtonSize = 'small' | 'default' | 'large';
...
export interface WarningButtonProps {
   ...
   * - `ghost` — white background, orange border, dark-orange text/icon; hover shifts bg to orange-light
   */
  variant?: WarningButtonVariant;
  /** Size variant — controls height, padding, font, and icon sizes */
  size?: WarningButtonSize;
...
const USER_ICON: Record<WarningButtonSize, React.FC> = {
  small:   UserIconSmall,
  default: UserIconDefault,
  large:   UserIconLarge,
};

const ARROW_ICON: Record<WarningButtonSize, React.FC> = {
...
/**
 * WarningButton — status-specific button for warning / cautionary actions.
 *
 * Two variants:
 ...
 *
 * Storybook: Atoms / Buttons / StatusButton / WarningButton
 */
export const WarningButton: React.FC<WarningButtonProps> = ({
  ...
  const classNames = [
    styles['btn-warning'],
    styles[`btn-warning--${variant}`],
    size !== 'default' ? styles[`btn-warning--${size}`] : '',
    loading ? styles['btn-warning--loading'] : '',
    disabled ? styles['btn-warning--disabled'] : '',
    className,
  ]
    .filter(Boolean)
    .join(' ');
  ...
  return (
    ...
    >
      {loading ? (
        <span className={styles['btn-warning__spinner']} aria-hidden="true" />
      ) : (
        <span className={styles['btn-warning__icon-leading']}>
          <UserIcon />
        </span>
      )}
      <span className={styles['btn-warning__label']}>{label}</span>
      {!loading && (
        <span className={styles['btn-warning__icon-trailing']}>
          <ArrowIcon />
        </span>
      )}
    </button>
